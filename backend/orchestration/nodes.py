...
from services.generation.providers import get_video_gen_provider
...
async def generate_wan_clip(state: EpisodeState) -> EpisodeState:
    """Node 8: Generate the intro clip using the user-selected provider.

    The user picks the video generation tool per episode (wan | cogvideox |
    colab | manim) in the UI. This node uses exactly that provider — NOT a
    fixed chain. If the selected provider fails, it falls back to the Manim
    title card (the universal safety net) so a render always exists.

    When the selected provider is the Colab worker and it fails/times out, a
    runnable .ipynb is exported (Pattern B) so the clip can be generated by
    hand on a Colab GPU.
    """
    ...
    try:
        ...
        # Resolve the provider: per-episode user selection, else global default.
        selected = (state.get("video_gen_provider") or "").strip().lower()
        provider_name = selected or get_settings().video_gen_provider

        # "manim" is a direct choice — skip video-gen entirely.
        if provider_name == "manim":
        ...
        # Use the user-selected provider directly.
        try:
            provider = get_video_gen_provider(provider_name)
            ...
            logger.warning(
                "selected_video_gen_failed_using_manim",
                provider=provider_name,
                error=str(e),
            )
            # If the selected provider was the Colab worker, export a manual
            # notebook (Pattern B) so the clip can be generated by hand.
            if provider_name == "colab":
                try:
                    from services.generation.notebook_exporter import NotebookExporter
                    nb_path = NotebookExporter().export(
                        state["episode_id"],
                        prompt,
                        negative_prompt,
                        duration_secs=8,
                    )
                    state["colab_notebook_path"] = str(nb_path)
                    logger.info(
                        "colab_notebook_exported_for_manual_run",
                        path=str(nb_path),
                    )
                except Exception as nb_err:
                    logger.warning("colab_notebook_export_failed", error=str(nb_err))
            # Safety net: Manim title card
            path = await wan_fallback.render(
                state["topic"],
                state["episode_id"],
                output_path,
