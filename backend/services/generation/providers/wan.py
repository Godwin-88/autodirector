"""Wan 2.1 provider via Alibaba DashScope (legacy, backward compatible).

This is a refactor of the original ``WanClient`` into the
``VideoGenProvider`` interface. Behavior is unchanged; the rest of the
pipeline can now swap it out for CogVideoX.
"""
import asyncio
...
logger = get_logger("providers.wan")


class WanProvider(VideoGenProvider):
    """Wan 2.1 text-to-video via DashScope."""
        ...
        for attempt in range(20):
            ...
            if status == "SUCCEEDED":
                ...
                if video_url:
                    ...
                    return output
                raise VideoGenAPIError("No video_url in succeeded response")
            elif status == "FAILED":
                raise VideoGenAPIError(f"Job failed: {status_data}")
            elif status in ("PENDING", "RUNNING"):
                await asyncio.sleep(10)
            else:
                raise VideoGenAPIError(f"Unknown status: {status}")
        raise VideoGenTimeoutError(f"Job {task_id} did not complete in 200s")
    ...
    async def generate(
        ...
        job_id = await self.submit_job(
            prompt, negative_prompt, duration_secs, resolution
        )
...
register_provider("wan", WanProvider)